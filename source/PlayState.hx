...
class PlayState extends MusicBeatState
{
	...
	public static var startMania:Int = 3;
	...
	// sway angle
	private var swayTimes:Array<Float> = [0.0, 0.0];
	private var swayAngle:Array<Float> = [0.0, 0.0];
	private var swaySpeed:Array<Float> = [0.0, 0.0];
	...
	override public function create()
	{
		...
		#if !debug
		beWatermark.text = "Friday Night Funkin': v" + MainMenuState.fridayVersion;
		peWatermark.text = "VS Doflanico " + MainMenuState.modVer;
		#end

		/*#if debug
		beWatermark.text = "Debug Build - FNF VS Doflanico v" + MainMenuState.beatEngineVersion;
		peWatermark.text = "Appuyez sur F2 pour ouvrir les logs";
		#end*/
	...
	override public function update(elapsed:Float)
	{
		...
		// and also sway camera funnies
		var cameras = [camGame, camHUD];
		for(i in 0...swayTimes.length){
			cameras[i].angle = Math.sin(Conductor.songPosition * (swaySpeed[i]/1000)) * swayAngle[i];
			swayTimes[i] -= elapsed;
			if(swayTimes[i] <= 0 && (swaySpeed[i] != 0.0 || swayAngle[i] != 0.0)){
				swaySpeed[i] = 0.0;
				swayAngle[i] = 0.0;
			}
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'Turn on cam zooming':
				camZooming = true;
			
			case 'Turn off cam zooming':
				camZooming = false;
